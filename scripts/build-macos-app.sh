...
APP_NAME="Git Auto Sync"
BUNDLE_ID="tools.franks.git-auto-sync"
...
usage() {
  cat <<EOF
Builds a distributable macOS .app bundle for Git Auto Sync.
...
GUI_PUBLISH_DIR="$ROOT_DIR/.artifacts/publish/gui/$RID"
DAEMON_PUBLISH_DIR="$ROOT_DIR/.artifacts/publish/daemon/$RID"
APP_BUNDLE_DIR="$OUTPUT_DIR/$APP_NAME.app"
APP_CONTENTS_DIR="$APP_BUNDLE_DIR/Contents"
APP_MACOS_DIR="$APP_CONTENTS_DIR/MacOS"
APP_RESOURCES_DIR="$APP_CONTENTS_DIR/Resources"
ICON_PNG="$ROOT_DIR/GitAutoSync.GUI/Assets/app-icon.png"
...
dotnet publish "$ROOT_DIR/GitAutoSync.GUI/GitAutoSync.GUI.csproj" \
  -c "$CONFIGURATION" \
  -r "$RID" \
  --self-contained true \
  -p:PublishAot=false \
...
echo "==> Publishing daemon ($RID, $CONFIGURATION)"
dotnet publish "$ROOT_DIR/GitAutoSync.Daemon/GitAutoSync.Daemon.csproj" \
  -c "$CONFIGURATION" \
  -r "$RID" \
  --self-contained true \
...
cp -R "$GUI_PUBLISH_DIR"/* "$APP_MACOS_DIR/"
cp "$DAEMON_PUBLISH_DIR/GitAutoSync.Daemon" "$APP_MACOS_DIR/GitAutoSync.Daemon"
if [[ -f "$ROOT_DIR/GitAutoSync.Daemon/appsettings.json" ]]; then
  cp "$ROOT_DIR/GitAutoSync.Daemon/appsettings.json" "$APP_MACOS_DIR/appsettings.json"
fi
chmod +x "$APP_MACOS_DIR/GitAutoSync.GUI" "$APP_MACOS_DIR/GitAutoSync.Daemon"
...
echo "Auto-login can be enabled from inside the app: Preferences -> Start at Login"
